...
You can change the password for MongoDB in [`config/daacs.properties`](config/daacs.properties) and [`install/mongo-user.script.js`](install/mongo-user.script.js). However, we recommend blocking access to the MongoDB to all IP addresses.
...
If the API starts successfully, you should be able to go to http://IP_ADDRESS:8080/swagger-ui.html and see the API documentation.
...
## DAACS Admin Dashboard

This uses R Shiny server.

```{bash, eval=FALSE}
cd /daacs/install
./install-daacs-dashboard.sh
```
...
mongo.user <- 'admin'
mongo.pass <- 'admin%40daacs'

daacs.domain <- 'umgc.daacs.net'
...
Go to this URL to get the metadata:

https://DAACS_DOMAIN/api/saml/metadata

Note that the meta data may have references to http://localhost:8080. These will need to be changed to use the appropriate domain and mapping through NGINX. 
